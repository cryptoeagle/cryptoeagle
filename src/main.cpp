...
// Copyright (c) 2013 CryptoEagle developers
...
const string strMessageMagic = "CryptoEagle Signed Message:\n";
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    if(nHeight < 120) {nSubsidy = 0.5 * COIN;} // minimal rewards to prevent instamine
...
static const int64 nInterval = nTargetTimespan / nTargetSpacing;	// 30 blocks difficulty retarget
/////////////
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CryptoEagleMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CryptoEagleMiner : ProcessBlock, block not accepted");
    }
...
{
    printf("CryptoEagleMiner started\n");
    ...
    try { loop {
        ...
        printf("Running CryptoEagleMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("CryptoEagleMiner terminated\n");
        throw;
    }
}
